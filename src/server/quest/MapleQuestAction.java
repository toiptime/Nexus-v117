...
public class MapleQuestAction implements Serializable {
    ...
    private static List<Integer> getJobBy5ByteEncoding(int encoded) {
        ...
        if ((encoded & 0x20000) != 0) {
            ret.add(2001); // I'm not sure of this one
            ret.add(2200);
        }
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // Remove items
                        try {
                            ...
                            System.err.println("[Hack] Completing a quest without meeting the requirements" + ie);
                        }
            ...
            case item: {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // Remove items
                        ...
                        final int period = item.period / 1440; // I'm guessing.
                        final String name = MapleItemInformationProvider.getInstance().getName(id);
                        if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                            final String msg = "You have attained the Title <" + name + ">";
                            c.print(-1, msg);
