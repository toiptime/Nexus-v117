...
function start(mode, type, selection) {
	...
	if (status == 0) {
	    qm.sendNext("Hey Man, What's up? I am Roger who can teach you adorable new Maplers lots of information.");
	} else if (status == 1) {
	    qm.sendNextPrev("You are asking who made me do this?\r\n It's myself! I wanted to do this and just be kind to you new travellers.");
	} else if (status == 2) {
	    qm.askAcceptDecline("So..... Let me just do this for fun!");
	} else if (status == 3) {
...
//		qm.getPlayer().updateSingleStat(client.MapleStat.HP, 25);
	    ...
	    qm.sendNext("Surprised? If your HP becomes 0, then you are in trouble. Now, I will give you #rRoger's Apple#k. Please take it. You will feel stronger. Open the Item window and double click to consume. Hey, it's very simple to open the Item window. Just press #bI#k on your keyboard.");
	} else if (status == 4) {
	    qm.sendPrev("Please take all Roger's Apples that I gave you. You will be able to see the HP bar increasing. Please talk to me again when you recover your HP to 100%.");
	} else if (status == 5) {
	    qm.forceStartQuest();
	    qm.dispose();
	}
