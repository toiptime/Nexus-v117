...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (record == selection && cm.getDojoPoints() >= required) {
		    ...
		    cm.sendOk("You either already have the item or you have insufficient training points. Do try getting the weaker belts first.");
		}
		cm.dispose();
	    } else if (sel == 1) {
	...
	if (status == 0) {
		...
		cm.askAcceptDecline("Do you want to quit and leave this place?");
	    } else if (sel == 2) {
		if (cm.getParty() == null) {
