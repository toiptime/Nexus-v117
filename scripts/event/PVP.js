importClass(Packages.tools.packet);
...
function playerEntry(eim, player) {
    ...
    if (eim.getProperty("started").equals("0")) {
        ...
        if (type != 0) {
            player.getClient().getSession().write(CField.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        }
...
function broadcastType(eim) {
    ...
    eim.broadcastTeamPacket(CField.getPVPType(type, c1, 1, !eim.getProperty("started").equals("0"), lvl), 0);
    eim.broadcastTeamPacket(CField.getPVPTeam(c1), 0);
	
    eim.broadcastTeamPacket(CField.getPVPType(type, c2, 2, !eim.getProperty("started").equals("0"), lvl), 1);
    eim.broadcastTeamPacket(CField.getPVPTeam(c2), 1);
}
...
function broadcastType(eim, player) {
    ...
    eim.broadcastTeamPacket(CField.getPVPType(type, player.getTeam() == 0 ? c1 : c2, player.getTeam() + 1, !eim.getProperty("started").equals("0"), lvl), player.getTeam());
    eim.broadcastTeamPacket(CField.getPVPTeam(player.getTeam() == 0 ? c1 : c2), player.getTeam());
}
...
function addPVPScore(eim, player, score) {
    ...
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        ...
        player.getClient().getSession().write(CField.getPVPIceGage(xx));
...
function updateScoreboard(eim, score) {
    ...
    if (score == true || score == null) {
        ...
        if (ty != 0 && ty != 2) {
            eim.broadcastPacket(CField.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            players.get(i).getClient().getSession().write(CField.getPVPResult(c, exp, winningTeam + 1, players.get(i).getTeam() + 1));
        }
    }
}
...
function end(eim) {
    ...
    eim.broadcastPacket(CField.getPVPMode(6));
    eim.broadcastPacket(CField.enablePVP(false));
    updateScoreboard(eim, false);
    var players = eim.getPlayers();
    for (var i = 0; i < players.size(); i++) {
        players.get(i).getClient().getSession().write(CWvsContext.clearMidMsg());
