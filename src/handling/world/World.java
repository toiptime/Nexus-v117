...
public class World {
    ...
    public static void registerRespawn() {
        ...
        // 3000 good or bad? I've no idea >_>
    }
                ...
                if (ch > 0) {
                    ...
                    if (chr != null && !chr.getName().equalsIgnoreCase(namefrom)) { //Extra check just in case
                        ...
                        if (chr.getClient().isMonitored()) {
                            World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[Game Master Message] " + namefrom + " said to " + chr.getName() + " (Party): " + chattext));
                        }
                    }
                }
            }
            ...
            for (int characterId : recipientCharacterIds) {
                ...
                if (ch > 0) {
                    ...
                    if (chr != null && chr.getBuddylist().containsVisible(cidFrom)) {
                        ...
                        if (chr.getClient().isMonitored()) {
                            World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[Game Master Message] " + nameFrom + " said to " + chr.getName() + " (Buddy): " + chattext));
                        }
                    }
                }
            }
