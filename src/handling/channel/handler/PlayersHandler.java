...
public class PlayersHandler {
    ...
    public static final void buySilentCrusade(final LittleEndianAccessor slea, final MapleClient c) {
        // UI window is 0x49
        // slea: [00 00] [4F 46 11 00] [01 00]
        short slot = slea.readShort(); //slot of item in the silent crusade window
        // Note: slot is probably used to check if item is equip or scroll but ut can be done with itemid too
    ...
    public static final void updateRedLeafHigh(LittleEndianAccessor slea, MapleClient c) { // Not finished yet
        ...
        if (joejoe < 0 || hermoninny < 0 || littledragon < 0 || ika < 0) {
            CWvsContext.enableActions();
            return;
        }
        if (joejoe > 20 || hermoninny > 20 || littledragon > 20 || ika > 20) {
            CWvsContext.enableActions();
            return;
        }
        if (!GameConstants.isRedLeaf(c.getPlayer().getMapId())) {
            CWvsContext.enableActions();
            return;
        }
