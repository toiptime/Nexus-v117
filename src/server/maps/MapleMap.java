...
public final class MapleMap {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        final int mobpos = mob.getTruePosition().x, cmServerrate = ChannelServer.getInstance(channel).getMesoRate(chr.getWorld()), chServerrate = ChannelServer.getInstance(channel).getDropRate(chr.getWorld()), caServerrate = ChannelServer.getInstance(channel).getCashRate();
        Item idrop;
        byte d = 1;
        ...
        for (final MonsterGlobalDropEntry de : globalEntry) {
            if (Randomizer.nextInt(1000000) < de.chance && (de.continent < 0 || (de.continent < 1 && mapid / 100000000 == de.continent) || (de.continent < 1000 && mapid / 10000000 == de.continent) || (de.continent < 10 && mapid / 1000000 == de.continent))) {
                if (de.questid > 0 && chr.getQuestStatus(de.questid) != 1) {
                    continue;
                }
                if (de.itemId == 0) {
                    chr.modifyCSPoints(1, (int) ((Randomizer.nextInt(cashz) + cashz + cashModifier) * (chr.getStat().cashBuff / 100.0) * chr.getCashMod() * 0.125), true);
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        if (monster.getId() == 9400902) {
            chr.getMap().startMapEffect("So what do you think? Just as though as the original, right? Move on by going to Resercher H.", 5120039);
        }
        ...
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "To the crew that have finally conquered Horntail after numerous attempts, I salute thee! You are the true heroes of Leafre!"));
            charactersLock.readLock().lock();
            try {
            ...
            doShrine(true);
        } else if (mobid == 8810122 && mapid == 240060201) { // Chaos Horntail
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "To the crew that have finally conquered Chaos Horntail after numerous attempts, I salute thee! You are the true heroes of Leafre!"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
...
//            FileoutputUtil.log(FileoutputUtil.Horntail_Log, MapDebug_Log());
            ...
            doShrine(true);
        } else if (mobid == 8870100 && mapid == 262031300) { // Dark Hilla
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "Dark Hilla is dead."));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(70);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            ...
            doShrine(true);
        } else if (mobid == 9400121 && mapid == 801040003) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(12);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 9400300 && mapid == 801040100) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(65);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            ...
            doShrine(true);
        } else if (mobid == 9420544 && mapid == 551030200) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(66);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 9420549 && mapid == 551030200) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(67);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            ...
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(68);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 8500001 && mapid == 220080001) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(13);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 8510000 && mapid == 230040420) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(14);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 9300119 && mapid == 925100500) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(155);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 9300468 && mapid == 923040400) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(156);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 9300215 && mapid == 925064700) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(167);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
            doShrine(true);
        } else if (mobid == 9400409 && mapid == 800040410) {
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
                    c.finishAchievement(120);
                }
            } finally {
                charactersLock.readLock().unlock();
            }
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
    ...
    private String MapDebug_Log() {
        ...
        sb.append(" | Map ID : ").append(this.mapid);
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                    c.getSession().write(MobPacket.spawnAswanMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid)); // TODO: effect
                } else {
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                    c.getSession().write(MobPacket.spawnAswanMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0));
                } else {
                    c.getSession().write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0));
                }
            }
    ...
    public final int spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos) {
        try {
            ...
            spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
                ...
                public final void sendPackets(MapleClient c) {
                    if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                        c.getSession().write(MobPacket.spawnAswanMonster(monster, effect, 0));
                    } else {
    ...
    public final void spawnFakeMonster(final MapleMonster monster) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAswanMap(c.getPlayer().getMapId())) {
                    c.getSession().write(MobPacket.spawnAswanMonster(monster, -4, 0));
                } else {
...
//	broadcastMessage(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (!chr.isInBlockedMap() && chr.getLevel() > 10 && isTown() && chr.getMapId() % 1000000 == 0) {
            chr.getClient().getSession().write(CField.getQuickMoveInfo());
        }
        ...
        if (mapid == 910000000) {
            final boolean FM_CHANGE_MUSIC = false;
            final String FM_BGM = "Bgm03/Elfwood";
            final boolean FM_MAP_EFFECT = false;
            if (FM_CHANGE_MUSIC && FM_BGM != null) {
                chr.getClient().getSession().write(CField.musicChange(FM_BGM));
            }
            if (FM_MAP_EFFECT) {
    ...
    public String spawnDebug() {
        StringBuilder sb = new StringBuilder("Monsters In Map : ");
        sb.append(this.getMobsSize());
        sb.append(" Spawned Monsters On Map: ");
    ...
    public void respawn(final boolean force, final long now) {
        ...
        if (force) { //cpq quick hack
            final int numShouldSpawn = monsterSpawn.size() * MoonlightRevamp.monsterSpawn - spawnedMonstersOnMap.get();
            ...
            final int numShouldSpawn = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() * MoonlightRevamp.monsterSpawn : maxRegularSpawn * MoonlightRevamp.monsterSpawn) - spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
                ...
                for (Spawns spawnPoint : randomSpawn) {
                    ...
                    if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid) || (monsterSpawn.size() * MoonlightRevamp.monsterSpawn < 10 && maxRegularSpawn * MoonlightRevamp.monsterSpawn > monsterSpawn.size() * MoonlightRevamp.monsterSpawn && partyBonusRate > 0)) {
                        spawnPoint.spawnMonster(this);
                        spawned++;
                    }
                    if (spawned >= numShouldSpawn) {
    ...
    public void removeDrops(MapleCharacter player, boolean command) {
        List<MapleMapObject> items = player.getMap().getMapObjectsInRange(player.getPosition(), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.ITEM));
        for (MapleMapObject i : items) {
            player.getMap().removeMapObject(i);
            player.getMap().broadcastMessage(CField.removeItemFromMap(i.getObjectId(), 0, player.getId()));
    ...
    private void broadcastNONGMMessage(MapleCharacter source, byte[] packet) {
        ...
        try {
            if (source == null) {
                ...
                for (MapleCharacter chr : characters) {
                    if (chr != source && (chr.getGMLevel() <= 2)) {
                        chr.getClient().getSession().write(packet);
                    }
                }
            }
