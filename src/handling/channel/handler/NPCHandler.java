...
public class NPCHandler {
    ...
    public static final void OpenQuickMove(final LittleEndianAccessor slea, final MapleClient c) {
        final int npcid = slea.readInt();
        if (c.getPlayer().hasBlockedInventory() || c.getPlayer().isInBlockedMap() || c.getPlayer().getLevel() < 10) {
            return;
        }
        for (int i = 0; i < GameConstants.quickMoveNpcIds.length; i++) {
            if (GameConstants.quickMoveNpcIds[i] == npcid) { // For now
