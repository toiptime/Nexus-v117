...
public class MapleShop {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (GameConstants.isThrowingStar(rs.getInt("itemid")) || GameConstants.isBullet(rs.getInt("itemid"))) {
                    MapleShopItem starItem = new MapleShopItem(rs.getShort("buyable"), rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), rs.getByte("rank"), rs.getInt("category"), rs.getInt("minLevel"), rs.getInt("expiration"));
                    ret.addItem(starItem);
                    if (rechargeableItems.contains(starItem.getItemId())) {
                        recharges.remove(Integer.valueOf(starItem.getItemId()));
                    }
                } else {
                    ret.addItem(new MapleShopItem(rs.getShort("buyable"), rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), rs.getByte("rank"), rs.getInt("category"), rs.getInt("minLevel"), rs.getInt("expiration")));
                }
            }
    ...
    public static MapleShop moonlightShop(int npc) { // No ranks / rechargable items for now
        ...
        for (MoonlightShop item : MoonlightShop.values()) {
            moonlight.addItem(new MapleShopItem((short) item.getBuyable(), item.getItemId(), item.getPrice(), item.getReqItem(), item.getReqItemQ(), item.getRank(), item.getCategory(), item.getMinLevel(), item.getExpiration()));
        }
        return moonlight;
    }
    ...
    public void recharge(final MapleClient c, final byte slot) {
        ...
        if (item.getQuantity() < slotMax) {
            ...
            if (c.getPlayer().getMeso() >= price) {
                ...
                c.getSession().write(InventoryPacket.updateInventorySlot(MapleInventoryType.USE, item, false));
