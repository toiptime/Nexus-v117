...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
    ...
    public PlayerNPC(ResultSet rs) throws Exception {
        ...
        job = rs.getShort("job");
        elf = rs.getInt("elf");
        demonmarking = rs.getInt("demonmarking");
    ...
    public void update(MapleCharacter chr) {
        if (chr == null || charId != chr.getId()) {
            return; // Can't use name as it mightve been change actually..
        }
        setName(chr.getName());
        setHair(chr.getHair());
        setFace(chr.getFace());
        ...
        setJob(chr.getJob());
        setElf(chr.getElf());
        setDemonMarking(chr.getDemonMarking());
    ...
    public void setDemonMarking(int demonmarking) {
        this.demonmarking = demonmarking;
    }
