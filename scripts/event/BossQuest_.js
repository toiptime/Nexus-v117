...
function allMonstersDead(eim) {
	...
	map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[Boss Party Quest] Congratulations! Your party has defeated all the bosses with " + points + " points!"));
	map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[Boss Party Quest] The points have been awarded, spend them as you wish."));
	disbandParty();
    } else {
	map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[Boss Party Quest] Your party now has " + points + " points! The next boss will spawn in 10 seconds."));
...
function beginQuest(eim) {
    var map = eim.getMapInstance(mapid, 0);
    map.broadcastMessage(tools.MaplePacketCreator.serverNotice(6, "[Boss Party Quest] The creatures of the darkness are coming in 30 seconds. Prepare for the worst!"));
